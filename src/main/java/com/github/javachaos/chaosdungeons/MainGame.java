...
public class MainGame {
  ...
  private void loop(GameLoop gameLoop) {
    ...
    while (!glfwWindowShouldClose(window)) {
      ...
      double dt = (double) gameLoop.getTargetTime() / 1000.0;
      gameLoop.update(dt);
      long elapsedTime = java.lang.System.nanoTime() - startTime;
      gameLoop.setTargetTime(elapsedTime);
      dt = (double) gameLoop.getTargetTime() / 1000.0;
      gameLoop.render(dt);
      // Calculate the time taken for update and render
      // Calculate the time to sleep to achieve the target FPS
      long sleepTime = gameLoop.getTargetTime() - elapsedTime;
